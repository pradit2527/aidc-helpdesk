...
/**
 * จำนวนครั้งที่กรอกผิดก่อนถูกล็อกตามนโยบายข้อ 3.2
 *
 * ใช้แสดงข้อความอธิบายเท่านั้น การตัดสินว่าล็อกหรือไม่อยู่ที่เซิร์ฟเวอร์
 * ซึ่งตอบ 423 กลับมา ฝั่งนี้นับเองไม่ได้เพราะตัวนับจริงอยู่ในฐานข้อมูล
 * และรีเฟรชหน้าก็ไม่รีเซ็ต ต่างจากตัวแปรในหน่วยความจำของแท็บ
 *
 * ⚠️ บัญชีที่ถูกล็อกไม่ปลดเองตามเวลา ต้องให้ Service Desk ปลดให้
 *    จึงไม่มีการนับถอยหลังใด ๆ ในหน้านี้
 */
const MAX_TRIES = 5;
      ...
      <section className="login-hero hatch relative flex flex-col justify-between gap-9 p-7 sm:p-12 lg:p-14 xl:p-16">
        ...
        <div className="flex flex-1 flex-col justify-center py-2">
          ...
          <h1 className="display-xl">
            AIDC Tech
            <br />
            Support <span className="amp">&amp;</span> Service
          </h1>
          <p
        ...
        <div className="mx-auto w-full max-w-[416px]">
          ...
          <form onSubmit={onSubmit} noValidate className="mt-7 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {/*
                นับเฉพาะจำนวนครั้ง ไม่บอกว่า "ครบเท่านี้แล้วจะถูกล็อก"

                การล็อกบัญชีตัดสินที่เซิร์ฟเวอร์ ไม่ใช่ที่นี่ และตอนนี้ถูกปิดไว้
                ด้วย LOCKOUT_ENABLED=false ข้อความเดิมจึงบอกสิ่งที่ไม่เกิดขึ้นจริง
                ผู้ใช้เห็น "7/5" แล้วสับสนว่าทำไมยังไม่ถูกล็อก
                ถ้าเซิร์ฟเวอร์ล็อกจริงจะตอบ 423 แล้วบล็อก locked ด้านล่างขึ้นเอง
              */}
              {tries > 0 && !locked && (
                <p className="text-[11.5px] leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                  ພະຍາຍາມເຂົ້າສູ່ລະບົບຜິດແລ້ວ{' '}
                  <span className="tabular font-semibold" style={{ color: 'var(--primary-hover)' }}>
                    {tries}
                  </span>{' '}
                  ຄັ້ງ — ກວດຊື່ຜູ້ໃຊ້ ແລະ ລະຫັດຜ່ານອີກຄັ້ງ
                </p>
              )}
            </div>
            ...
            {locked && (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  >
                    ປ້ອນລະຫັດຜ່ານຜິດຄົບ {MAX_TRIES} ຄັ້ງຕາມນະໂຍບາຍລະຫັດຜ່ານ ຂໍ້ 3.2 —
                    ບັນຊີຈະ<span className="font-semibold">ບໍ່ປົດລັອກເອງຕາມເວລາ</span>{' '}
                    ຕ້ອງຕິດຕໍ່ Service Desk ພ້ອມລະຫັດພະນັກງານເພື່ອຢືນຢັນຕົວຕົນ
                  </p>
                </div>
              </div>
            )}
