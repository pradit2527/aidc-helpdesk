...
|---|---|---|---|---|
| **SOP-01** | การรับแจ้งและบริหาร Ticket | 1) รับแจ้งและบันทึกเป็น Ticket ทันที (โทร/Walk-in เจ้าหน้าที่สร้างให้) 2) จำแนก Incident/Service Request + เลือกหมวดหมู่ 3) ประเมิน P1–P4 ตาม Priority Matrix 4) แก้เบื้องต้นจาก KB 5) เกินขีดความสามารถ → Escalate Tier 2/3 พร้อมสรุปสิ่งที่ตรวจแล้ว 6) อัปเดตสถานะตามรอบ SLA 7) บันทึกสาเหตุ+วิธีแก้ → `Resolved` + อีเมลให้ยืนยัน 8) ปิดเมื่อยืนยัน หรือครบ 3 วันทำการ + ส่ง CSAT | **Workflow:** state machine **แยกสองชุดตาม `ticket_type`** (v3 — ดู `02-data-model.md` §6) · ข้อ 2 "จำแนก Incident/Service Request" จึงเป็นการเลือก**เครื่องสถานะ** ไม่ใช่แค่ป้ายกำกับ · หมวดหมู่ในข้อเดียวกันถูกกรองด้วย `ticket_category.ticket_type_scope`. **ฟิลด์:** `ticket_type`, `impact`, `urgency`, `priority` (คำนวณจากเมทริกซ์), `channel`, `support_tier`, `resolution_note` (บังคับก่อน `resolved`), `next_status_report_due_at`. **UI:** ปุ่ม "สร้างแทนผู้อื่น" (`created_by ≠ requester_id`), แผงแนะนำบทความ KB ตอนพิมพ์หัวข้อ, กล่องสรุปสิ่งที่ตรวจแล้ว (บังคับกรอกตอนยกระดับ Tier). **อัตโนมัติ:** email→ticket ingestion, อีเมลขอยืนยันตอน `resolved`, auto-close 3 วันทำการ + ส่ง CSAT | **MVP** |
...
|---|---|---|---|---|
| **SOP-04** Onboarding | `CHK-ONBOARD` | **5** | ภาคผนวก ก.1 | **บังคับ** — ห้ามเปลี่ยนสถานะเป็น `resolved` / **`fulfilled`** จนกว่าข้อที่ `is_required` ครบ |
...
2. `in_progress → resolved` (เหตุขัดข้อง) **และ `in_progress → fulfilled` (คำขอบริการ)** ถูกปฏิเสธด้วย `409` ถ้ายังมี item ที่ `is_required AND NOT is_done` (ข้อความ: ระบุข้อที่ยังไม่ครบ)
   > ⚠️ ต้องครอบ `fulfilled` ด้วย — checklist ตาม SOP (ONBOARDING / OFFBOARDING) ผูกกับรายการใน
   > service catalog ทั้งคู่ ซึ่งแปลว่ามันอยู่บนเรื่องชนิด **คำขอบริการ** เสมอ และคำขอบริการ
   > ไม่เดินผ่าน `resolved` อีกต่อไป ถ้าเช็คแค่ `resolved` ด่านนี้จะไม่มีใครเดินผ่านเลย
...
| **SOP-06** `SR-SW-NONSTD` | 1) Tier 2 ประเมิน (ความปลอดภัย/ลิขสิทธิ์/ค่าใช้จ่าย) → 2) **Head of IT** → 3) จัดซื้อ (เฉพาะกรณีมีค่าใช้จ่าย) | ซอฟต์แวร์ไม่อยู่ใน `approved_software` | เวลาระหว่างรออนุมัติ = สถานะ **`pending_approval`** (หยุดนับ ตาม SLA ข้อ 9) |
...
2. ขณะมีแถว `pending` → ticket อยู่สถานะ **`pending_approval`** → **หยุดนับ SLA** (SLA ข้อ 9)
   *(v2 เคยใช้ `pending_user` + `pending_reason='approval'` — เลิกใช้แล้ว ดู `02-data-model.md` §6)*
...
4. เมื่อขั้นใดเป็น `rejected` → ticket ไปสถานะ **`rejected`** (ไม่ใช่ `cancelled`) พร้อมเหตุผลจาก `comment` (บันทึกใน `ticket_status_history`)
   *แยกสองคำนี้เพื่อให้รายงานตอบได้ว่าใบไหน "ผู้แจ้งถอนเอง" และใบไหน "ถูกพิจารณาแล้วไม่อนุมัติ"*
5. อนุมัติครบทุกขั้น → ticket ไป **`assigned`** และ **นาฬิกา fulfillment เริ่มนับที่วินาทีนั้น**
   (`sla_clock_started_at = now()` · ล้าง `pending_duration_minutes` เป็น 0 · คำนวณ
   `resolution_due_at` ใหม่จาก `service_catalog_item.target_minutes`)
