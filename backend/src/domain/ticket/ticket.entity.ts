 ...
 * ── อำนาจมอบหมายงาน ─────────────────────────────────────────────────────
 *
 * โจทย์: ฝั่ง Helpdesk มีสองระดับ
 *   - หัวหน้าทีม (support_lead) รับงานเองได้ และมอบหมายให้คนในทีมตัวเองได้
 *   - ทีม support (support_agent) รับงานจาก ticket ได้ แต่ห้ามมอบหมายให้ใคร
 *
 * สองชั้นซ้อนกัน เพราะสิทธิ์ตอบได้แค่ "มอบหมายเป็นไหม" ไม่ได้ตอบ "มอบให้ใครได้"
 *   1. บทบาท → ticket.assign (support_lead ขึ้นไปถือ, support_agent ไม่ถือ)
 *   2. ข้อมูลทีม (support_team_member.is_lead) → บอกว่าเป็นหัวหน้าของทีมไหน
 *
 * ต้องผ่านทั้งสองชั้น: มีบทบาทหัวหน้าแต่ไม่ได้เป็นหัวหน้าทีมใดเลย มอบให้ใครไม่ได้
 * และสมาชิกที่ถูกตั้ง is_lead ทั้งที่ไม่มีบทบาทหัวหน้า ก็มอบให้ใครไม่ได้เช่นกัน
 */
...
/**
 * รับงานเอง ทำได้หรือไม่
 *
 * ทีม support รับได้เฉพาะเรื่องที่ "ยังไม่มีผู้รับผิดชอบ" — ถ้ารับเรื่องที่เพื่อนร่วมทีม
 * ถืออยู่ได้ หัวหน้าที่มอบงานให้คนหนึ่งจะถูกเพื่อนอีกคนดึงงานไปได้ด้วยการกดรับเอง
 * ซึ่งเท่ากับเลี่ยงข้อห้ามมอบหมาย หัวหน้าทีมและผู้ดูแลดึงงานกลับมาได้ (mayAssignToOthers)
 *
 * รับเรื่องที่ตนถืออยู่แล้วให้ผ่านที่ด่านนี้ — การปฏิเสธ "มอบให้คนเดิมซ้ำ" เป็นหน้าที่ของ entity
 */
export function maySelfAssign(
  actor: AssignmentActor,
  currentAssigneeId: number | null,
): boolean {
  if (currentAssigneeId === null || currentAssigneeId === actor.userId) return true;
  return mayAssignToOthers(actor);
}
 ...
 *
 * ลำดับการตัดสินสำคัญ
 *   1. รับงานเอง ผ่านเสมอ — สิทธิ์ ticket.assign_self และเงื่อนไข "เรื่องว่าง" (maySelfAssign)
 *      ถูกตรวจไปแล้วที่ use case และการรับงานเองไม่ใช่การใช้อำนาจเหนือคนอื่น
