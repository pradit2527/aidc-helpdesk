    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        await this.db.insert(appUser).values({
          ...
          authProvider: 'local',
          /*
           * ธงนี้ไม่บังคับอะไรอีกแล้ว — หน้า /change-password ถูกถอดออก
           * ตามที่องค์กรสั่ง จึงไม่มีการเด้งไปบังคับเปลี่ยนตอนเข้าครั้งแรก
           *
           * ยังตั้งเป็น true เพราะความหมายที่เหลืออยู่คือ "บัญชีนี้ยังใช้
           * รหัสตั้งต้นที่ผู้ดูแลตั้งให้" ซึ่งเป็นความจริงของบัญชีที่เพิ่งนำเข้า
           * และเป็นสิ่งที่ผู้ดูแลอยากเห็นในรายการผู้ใช้ การกลบเป็น false
           * จะทิ้งข้อมูลนั้นไปโดยไม่ได้อะไรกลับมา
           */
