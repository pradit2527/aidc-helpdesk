...
export class TicketsController {
  ...
  @ApiOperation({
    ...
    description: [
      ...
      '- `support_lead` / `support_agent` / `company_admin` / `manager_viewer` เห็นตามบริษัทที่ได้รับสิทธิ์',
  ...
  @ApiOperation({
    ...
    description: [
      '- รับเอง (`assignee_id` = ตัวเอง) ต้องมี `ticket.assign_self` (ทีม support ทุกคนมี)',
      '  ทีม support รับได้เฉพาะเรื่องที่ **ยังไม่มีผู้รับผิดชอบ** — เรื่องที่มีคนถืออยู่แล้วตอบ',
      '  `403 ALREADY_ASSIGNED` เว้นแต่ผู้เรียกเป็นหัวหน้าทีม/ผู้ดูแล (ดึงงานกลับมาได้)',
      '- **มอบให้คนอื่น** ต้องมี `ticket.assign` (บทบาท `support_lead` ขึ้นไป) **และ** อย่างใดอย่างหนึ่ง',
      ...
      '',
      '  ⚠️ ต้องมีทั้งสองอย่าง: บทบาทหัวหน้าทีม (`support_lead` → ถือ `ticket.assign`)',
      '  และเป็นหัวหน้าของทีมนั้นจริง (`is_lead`) — บทบาทให้อำนาจ ทีมบอกว่ามอบให้ใครได้',
      '',
