 ...
 * ── รหัสผ่านผิด ─────────────────────────────────────────────────────
 *
 * ⚠️ ใช้ชื่อผู้ใช้ที่ไม่มีจริง ห้ามยิงใส่บัญชีผู้ดูแลตัวจริงเด็ดขาด
 *
 * เดิมเทสต์นี้ยิงรหัสผิดใส่ SEED_ADMIN_USERNAME แล้วค่อยล็อกอินถูก
 * ทีหลังเพื่อล้างตัวนับ ซึ่งใช้ได้ตราบใดที่เทสต์รันจบทั้งไฟล์
 *
 * แต่พอรันแล้วโดน rate limit ตั้งแต่ต้น เทสต์จะไม่ไปถึงขั้นล็อกอินสำเร็จ
 * ตัวนับจึงค้างไว้ พอรันซ้ำหลายรอบก็ครบ 5 แล้วล็อกบัญชีผู้ดูแลจริง
 * ซึ่งปลดเองตามเวลาไม่ได้ตามนโยบาย 3.2 — ต้องเข้าไปแก้ฐานข้อมูลถึงจะกลับมาใช้ได้
 *
 * บัญชีที่ไม่มีจริงให้ผลลัพธ์เหมือนกันทุกประการอยู่แล้ว (นั่นคือสิ่งที่เทสต์
 * ถัดไปพิสูจน์) จึงครอบคลุมเท่ากันโดยไม่ต้องเสี่ยงล็อกบัญชีใคร
 * ส่วนกลไกนับและล็อกจริง ทดสอบที่ชั้นฐานข้อมูลใน db-smoke แทน
 */
const wrong = await call('/auth/login', {
  method: 'POST',
  body: JSON.stringify({ username: 'ບໍ່ມີບັນຊີນີ້-a', password: 'ຜິດແນ່ນອນ-1234' }),
});
...
const noUser = await call('/auth/login', {
  method: 'POST',
  body: JSON.stringify({ username: 'ບໍ່ມີບັນຊີນີ້-b', password: 'ຜິດແນ່ນອນ-5678' }),
});
check(
